...
any **push to `main`** only (not to any other branch). It is *highly* recommended that you do your development on a different branch and then merge that branch with `main` if you want the CI tests to run. To test locally on your branch use the `Makefile` or your IDE.
...
back to your Github repository (but only to the `main` branch). Do not change
...
**Note:** The CI uses this makefile so don't change it.
